#!/bin/bash
#
# Script: util_20_quick_fix_duplicates.sh
# Purpose: Quick fix for duplicate ACRN GRUB entries
# Usage: sudo ./util_20_quick_fix_duplicates.sh
...
if [ "$NEEDS_FIX" = true ]; then
    ...
    if [ -f "./util_17_clean_grub_entry.sh" ]; then
        ./util_17_clean_grub_entry.sh
    else
        echo "  Run manually: sudo ./util_17_clean_grub_entry.sh"
    fi
fi
